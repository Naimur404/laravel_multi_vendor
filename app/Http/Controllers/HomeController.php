...
class HomeController extends Controller
{
    ...
    public function userProfileUpdate(Request $request)
    {
        ...
        if($seller){
            ...
            $seller->nagad_number = $request->nagad_number;
    ...
    {
        $detailedProduct  = Product::with('reviews', 'brand', 'stocks', 'user', 'user.shop','flash_deal_product')->where('auction_product', 0)->where('slug', $slug)->where('approved', 1)->first();
        
        /* $productWithCampaignDetail  = Product::with('user','flash_deal_product')->where('slug', $slug)->first();
        $flash_deal_product = FlashDealProduct::where('product_id', $productWithCampaignDetail->id)->first();
        $flash_deal_boolean = false;
        if($flash_deal_product != null){
            $flash_deal_boolean = true;
        } */
        ...
        if($detailedProduct != null && $detailedProduct->published){
            ...
            if($detailedProduct->digital == 1){
                ...
                //return view('frontend.product_details', compact('detailedProduct','flash_deal_boolean'));
...
//        $categories = Category::where('level', 0)->orderBy('name', 'asc')->get();
    ...
    public function all_shops() {
        $shop_names = Shop::all()->pluck('name')->toArray();
        $shops = Shop::whereIn('name', $shop_names)->paginate(15);  //paginate don't work if it's not a builder method
        //$collections = Shop::all()->whereNotNull('name');
        //$shops = PaginationHelper::paginate($collections, 15);
        //ddd($shops);
    }

    public function official_shops() {
        $shops = Shop::whereIn('user_id', verified_sellers_id())->paginate(15);
        //ddd($shops);
        return view('frontend.official_shop_listing', compact('shops'));
    }
