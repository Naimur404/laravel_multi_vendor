<div class="modal-body">
  ...
  <p>Address: <i class="demo-pli-map-marker-2 icon-lg icon-fw mr-1"></i>{{ $seller->user->shop->address }}</p>
  <p>Shop: <a href="{{ route('shop.visit', $seller->user->shop->slug) }}" class="btn-link"><i class="demo-pli-internet icon-lg icon-fw mr-1"></i>{{ $seller->user->shop->name }}</a></p>
  <p><i class="demo-pli-old-telephone icon-lg icon-fw mr-1"></i>{{ $seller->user->phone }}</p>

  <h6 class="mb-4">{{translate('Payout Info')}}</h6>
  <p>{{translate('Bkash Number (MFS)')}} : {{ $seller->bkash_number }}</p>
  <p>{{translate('Nagad Number (MFS)')}} : {{ $seller->nagad_number }}</p></br>
