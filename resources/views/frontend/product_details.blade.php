                                    ...
                                    <div class="col-sm-10">
                                        <div class="product-quantity d-flex align-items-center">
                                            <div class="row no-gutters align-items-center aiz-plus-minus mr-3" style="width: 130px;">
                                                {{-- @if(!$flash_deal_boolean) --}}
                                                    <button class="btn col-auto btn-icon btn-sm btn-circle btn-light" type="button" data-type="minus" data-field="quantity">
                                                        <i class="las la-minus"></i>
                                                    </button> 
                                                {{-- @endif --}}

                                                <input type="number" name="quantity" id="front-input-quantity" class="col border-0 text-center flex-grow-1 fs-16 input-number" placeholder="1" value="{{ $detailedProduct->min_qty }}" min="{{ $detailedProduct->min_qty }}" max="{{ $detailedProduct->max_qty }}" onchange="handleQuantityChange({{ $detailedProduct->max_qty }})" readonly>

                                                <button id="quantity-plus-button" class="btn col-auto btn-icon btn-sm btn-circle btn-light" type="button" data-type="plus" data-field="quantity">
...
@section('script')
    <script type="text/javascript">
        ...
        function handleQuantityChange(maxQty){
            let elementInput = document.getElementById("front-input-quantity");
            let elementPlusButton = document.getElementById("quantity-plus-button");
            let elementInputNumberValue = parseInt(elementInput.value);
            //console.log(elementValue, maxQty);

            if(elementInputNumberValue >= maxQty){
                elementInput.value = maxQty;
                elementPlusButton.setAttribute("disabled", "disabled");
                alert("Maximum quantity order limit reached!");
            } else {
                elementPlusButton.removeAttribute("disabled");
            }
        }
